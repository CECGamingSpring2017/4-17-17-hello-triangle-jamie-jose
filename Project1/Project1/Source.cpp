...
int main() {
	...
	double Earth_Semimajor_Axis = 149.6;  //how far it goes to the right 
	double Mars_Semimajor_Axis = 227.92;  //how far it goes to the right 
	double Earth_Eccentricity = .067; // change the elipse
	double Mars_Eccentricity = 0.0935; // change the elipse
	double Period_Ratio_Earth = 1; // down and up
	...
	while (1)
	{
		...
		if (ev.type == ALLEGRO_EVENT_TIMER) {
			...
			//parametric equations for Earth's elliptical orbit	Mars_y = Mars_Semimajor_Axis*(sqrt((1 - Mars_Eccentricity)*(1 - Mars_Eccentricity))*sin((t*3.14) / 180)) + SCREEN_HEIGHT / 2;
		...
		if (redraw && al_is_event_queue_empty(event_queue)) {
			...
			//draw sun
			al_draw_filled_circle(sun_x, sun_y, 20, al_map_rgb(250, 250, 50));
			//draw earth
			al_draw_filled_circle(earth_x, earth_y, 10, al_map_rgb(234, 50, 200));
			al_draw_filled_circle(Mars_x, Mars_y, 7, al_map_rgb(255, 100, 150));
...
double Earth_Semimajor_Axis = 149.6;  //how far it goes to the right 
double Mars_Semimajor_Axis = 227.92;  //how far it goes to the right 
double Earth_Eccentricity = .67; // change the elipse
double Mars_Eccentricity = 0.0935; // change the elipse
double Period_Ratio_Earth = 1; // down and up
double Period_Ratio_Mars = .5317; //found by dividing orbital period of Earth by orbital period of Mars